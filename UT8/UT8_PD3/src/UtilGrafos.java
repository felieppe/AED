...
public class UtilGrafos {
    ...
    public static <T extends IGrafoDirigido> T cargarGrafo(String nomArchVert, String nomArchAdy, 
        ...
        Comparable[] datos = ManejadorArchivosGenerico.leerArchivo(nomArchAdy, ignoreHeader);
        TVertice vertice1 = new TVertice(datos[0]);
        TVertice vertice2 = new TVertice(datos[1]);
        double peso = Double.parseDouble((String) datos[2]);
        ...
        for (String aristaString : aristas) {
            if ((aristaString != null) && (aristaString.trim() != "")) {
                String[] datosArista = aristaString.split(",");
                aristasList.add(new TArista(vertice1, vertice2, peso));
            }
        }
        try {
